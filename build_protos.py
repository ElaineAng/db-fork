...
def compile_protos():
    ...
    proto_files = list(Path("dblib").glob("*.proto"))
    ...
    for proto_file in proto_files:
        ...
        try:
            ...
            print("Warning: protoc not found. Skipping proto compilation.", file=sys.stderr)
            print("  To compile protos, install protobuf compiler:", file=sys.stderr)
            print("    macOS: brew install protobuf", file=sys.stderr)
            print("    Ubuntu: apt-get install protobuf-compiler", file=sys.stderr)
...
class BuildWithProtos(_build_py):
    ...
    def run(self):
        ...
        if not compile_protos():
            print("Proto compilation failed, but continuing with build...", file=sys.stderr)
