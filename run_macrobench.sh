...
#
# Usage:
#   ./run_macrobench.sh <workflow> <backend> <db_scale> <sql_path>
#
# Arguments:
...
if [[ $# -ne 4 ]]; then
    echo "Usage: $0 <workflow> <backend> <db_scale> <sql_path>"
...
# Validate backend
VALID_BACKENDS="dolt neon kpg xata"
...
BASE_CONFIG="macrobench/configs/${WORKFLOW}.textproto"
...
python -m macrobench.runner --config "$TMP_CONFIG" --outdir "run_stats/${RUN_ID}"
